...
class TreeBasedScoreModel(ScoreModel):
    ...
    def fit(
        ...
        for i in range(y_dim):
            ...
            # --- FIX for TypeError ---
            # Create a copy of the model parameters to avoid modifying the original dictionary.
            # The 'n_jobs' from the class initialization takes precedence.
            params_for_fit = self._model_params.copy()
            params_for_fit.pop('n_jobs', None) # Remove 'n_jobs' if it exists in the dictionary.
            # --- End of Fix ---
            ...
            score_model_i = _fit_one_tree_model(
                ...
                n_jobs=self.n_jobs, # Pass 'n_jobs' explicitly here.
                **params_for_fit,   # Unpack the cleaned dictionary.
            )
