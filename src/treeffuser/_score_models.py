    ...
    **model_args,
    ...
    if model_name == 'lightgbm':
        ...
        if early_stopping_rounds is not None and eval_set is not None:
            callbacks = [lgb.early_stopping(early_stopping_rounds, verbose=model_args.get("verbose", 0) > 0)]
        model = lgb.LGBMRegressor(**model_args)
        model.fit(X=X, y=y, eval_set=eval_set, callbacks=callbacks, categorical_feature=model_args.get("cat_idx"))
        return model
    ...
    # --- XGBoost ---
        ...
        if early_stopping_rounds is not None and eval_set is not None:
            ...
            fit_params['verbose'] = model_args.get("verbose", 0) > 0
    ...
    # --- CatBoost ---
        ...
        import catboost
        # API translation: n_jobs -> thread_count
    ...
    # --- Scikit-learn Models ---
        ...
        if model_name == 'random_forest':
            ...
            # This model doesn't support n_jobs, so remove it if present
    ...
    # --- GPBoost ---
        ...
        early_stopping_rounds = model_args.pop('early_stopping_rounds', None)
        if early_stopping_rounds is not None and eval_set is not None:
            callbacks = [gpb.early_stopping(early_stopping_rounds, verbose=model_args.get("verbose", 0) > 0)]
...
class TreeBasedScoreModel(ScoreModel):
    ...
    def fit(
        ...
        for i in range(y_dim):
            ...
            # Start with a copy of user-provided hyperparams
            params_for_fit = self._model_params.copy()
            
            # Set/overwrite the global parameters from the class __init__
            params_for_fit['n_jobs'] = self.n_jobs
            params_for_fit['verbose'] = self.verbose

            # Handle random state naming conventions. Most libraries use 'random_state'.
            # CatBoost uses 'random_seed'. The fitter function will handle this.
        ...
        for model in self.models:
            if hasattr(model, 'best_iteration_'):
                ...
                # Fallback for models without a clear attribute
